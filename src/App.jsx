...
function JobDetails() {
  ...
  // ✅ UPDATED RENDER TABLE: Accepts 'skipCols' prop
  const RenderTable = ({ data, title, autoTotal = true, overrideFirstCol, showNote, skipCols = [] }) => {
    ...
    if (autoTotal) {
        headers.forEach(header => {
            // ✅ SKIP LOGIC: If column is in skipCols list, don't total it
            if (skipCols.includes(header)) return;

            // Don't calculate totals for Text Columns like Eligibility or Name
      ...
      {(job.stateWiseVacancy || job.zoneWiseGraduate || job.zoneWiseUG) && (
          <>
            {job.stateWiseVacancy && 
                <RenderTable 
                    data={job.stateWiseVacancy} 
                    title={job.vacancyTableTitle || "State Wise Vacancy Details"} 
                    overrideFirstCol={job.vacancyColumnName} 
                    showNote={job.stateTableNote || job.tableNote} 
                    skipCols={job.skipTotalFor} // ✅ Pass skipTotalFor prop
                />
            }
