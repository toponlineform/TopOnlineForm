...
import { Search, X, Share2 } from 'lucide-react'; // <--- Added Share2 Import
...
// --- Job Details (Updated with Share & Related Jobs) ---
function JobDetails() {
  ...
  return (
    <div className="job-container">
      ...
      <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginBottom: '20px' }}>
        <a href={whatsappUrl} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', background: '#25D366', color: 'white', padding: '8px 15px', borderRadius: '5px', fontWeight: 'bold', fontSize: '14px', textDecoration: 'none' }}>
           <Share2 size={16} style={{marginRight:'5px'}} /> Share on WhatsApp
        </a>
        <a href={telegramUrl} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', background: '#0088cc', color: 'white', padding: '8px 15px', borderRadius: '5px', fontWeight: 'bold', fontSize: '14px', textDecoration: 'none' }}>
           <Share2 size={16} style={{marginRight:'5px'}} /> Share on Telegram
        </a>
      </div>
      ...
      {job.examPattern && (<><div className="section-header">{(job.examPattern.pet || (job.examPattern.stages && job.examPattern.stages.some(s => s.type === 'pet'))) ? "Exam Pattern & Physical Test" : "Exam Pattern"}</div><div style={{padding: '10px'}}>{job.examPattern.details && (<ul style={{listStyleType: 'disc', marginLeft: '20px', marginBottom: '15px'}}>{job.examPattern.details.map((item, i) => <li key={i} style={{marginBottom: '5px'}}>{item}</li>)}</ul>)}{job.examPattern.table && <RenderSmartTable data={job.examPattern.table} />}{job.examPattern.tier1 && <RenderSmartTable data={job.examPattern.tier1} title="Tier-I Exam Pattern" />}{job.examPattern.tier2 && <RenderSmartTable data={job.examPattern.tier2} title="Tier-II Exam Pattern" />}{job.examPattern.cbt1 && <RenderSmartTable data={job.examPattern.cbt1} title={job.examPattern.cbt1Title || "CBT-1 Pattern"} />}{job.examPattern.cbt2 && <RenderSmartTable data={job.examPattern.cbt2} title={job.examPattern.cbt2Title || "CBT-2 Pattern"} />}{job.examPattern.generalistObjective && <RenderSmartTable data={job.examPattern.generalistObjective} title="1. Generalist - Objective Test" />}{job.examPattern.generalistDescriptive && <RenderSmartTable data={job.examPattern.generalistDescriptive} title="2. Generalist - Descriptive Test" />}{job.examPattern.specialistObjective && <RenderSmartTable data={job.examPattern.specialistObjective} title="3. Specialist - Objective Test" />}{job.examPattern.specialistDescriptive && <RenderSmartTable data={job.examPattern.specialistDescriptive} title="4. Specialist - Descriptive Test" />}{job.examPattern.pet && <RenderSmartTable data={job.examPattern.pet} title="Physical Efficiency Test (PET)" />}{job.examPattern.stages && job.examPattern.stages.map((stage, index) => (<RenderSmartTable key={index} data={stage.data} title={stage.title} />))}</div></>)}
      ...
      {/* --- NEW: Related Jobs Section --- */}
      {relatedJobs.length > 0 && (
        <div style={{ marginTop: '30px', borderTop: '2px solid #eee', paddingTop: '20px' }}>
          <div className="section-header" style={{background: '#333', marginTop: 0}}>Related Jobs</div>
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {relatedJobs.map(relJob => (
               <li key={relJob.id} style={{ padding: '10px', borderBottom: '1px solid #eee' }}>
                 <Link to={`/${relJob.slug}`} style={{ textDecoration: 'none', color: '#007bff', fontWeight: 'bold' }}>
                   👉 {relJob.shortTitle || relJob.title}
                 </Link>
               </li>
            ))}
          </ul>
