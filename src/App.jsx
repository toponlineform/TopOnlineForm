...
function JobDetails() {
  ...
  return (
    <div className="job-container">
      ...
      {/* 6. Salary Details Table (For jobs like AIIMS) */}
      {job.salaryDetails && (
        <>
            ...
            <table style={{minWidth: '100%'}}>
              ...
              <tbody>
                {job.salaryDetails.map((row, index) => (
                  <tr key={index}>
                    <td>{row.post}</td>
                    <td style={{fontWeight:'bold', color:'var(--sarkari-blue)'}}>{row.level}</td>
                  </tr>
                ))}
              </tbody>
            </table>
